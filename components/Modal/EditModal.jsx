...
export default function EditModal({ setIsOpen, content }) {
  const contentItem = content[0]

  const [newName, setNewName] = useState(contentItem.name)
  const [newPrice, setNewPrice] = useState(contentItem.price)
  const [newDiscription, setNewDiscription] = useState(contentItem.description)

  const submitHandler = async (event) => {
    event.preventDefault()

    if (newName === contentItem.name && newPrice === contentItem.price && newDiscription === contentItem.description) {
      toast.error('هیچ فیلدی ویرایش نشد !')
      setIsOpen(false)
    }
        ...
        <div className="bg-white p-6 rounded-lg shadow-lg sm:w-6/12 w-full mx-auto relative">
          <form className="flex flex-col items-center my-10 justify-center" onSubmit={submitHandler}>
            <p className='text-xl text-right mb-10'>
              ویرایش پروژه <span className='font-bold'>{contentItem.name}</span>
            </p>
